                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-zinc-200 font-medium">
                                            {(nextAppointment.services as any)?.[0]?.name || (nextAppointment.services as any)?.name}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div key={app.id} className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl flex items-center gap-4 hover:border-zinc-700 transition-all opacity-75 hover:opacity-100">
                                ...
                                <div className="flex-1">
                                    <h4 className="text-zinc-200 font-bold text-sm">
                                        {(app.services as any)?.[0]?.name || (app.services as any)?.name}
                                    </h4>
                                    <p className="text-zinc-500 text-xs flex items-center gap-1 mt-1">
                                        <User size={10} />
                                        {/* Tenta pegar o primeiro item da lista [0] OU pega o objeto direto */}
                                        {(app.barber as any)?.[0]?.full_name?.split(' ')[0] || (app.barber as any)?.full_name?.split(' ')[0]}
                                    </p>
                                </div>
                            </div>
                        ))}
